...
1.  How many cases are there in this data set?  How many variables?  For each 
    ...
    _There are 20,000 cases in this data set._  
    _There are 9 variables._  
    _genhlth is categorical and ordinal._  
    _exerany, hlthplan, smoke100, height, weight, wtdesire, and age are discrete._  
    _gender is categorical and nominal._
...
_More men have smoked > 100 cigarettes than women._
...
4.  What does this box plot show? Pick another categorical variable from the 
    ...
    _It shows the BMI broken down for each health value.  As one might expect, those of excellent health have the lowest BMI and poor has the worst._  
    ```{r}
    boxplot(bmi ~ cdc$smoke100)
    ```
    _I thought that those who smoke > 100 cigarettes would have a higher overall BMI, compared to those who haven't, but I suppose it's unsurprisingly not much of a difference.  The median and IQR seem identical, although there are more lower BMI outliers for smokers > 100, which isn't too surprising because smoking is an appetite suppressant._
...
-   Make a scatterplot of weight versus desired weight. Describe the 
    ...
    _Most of the plot is concentrated around the 100 - 200 range, which is not surprsing.  Also, some people have very modest weight goals at heavier weights, and one person at around 200lbs wants to be closer to 700._
...
-   What type of data is `wdiff`? If an observation `wdiff` is 0, what does 
    ...
    _It's a continuous type of data._  
    _If it's 0, then they are at their ideal weight, or at least content with their weight._  
    _If the diff is negative, then they want to lose weight.  If it's positive, they want to gain weight._  
...
-   Describe the distribution of `wdiff` in terms of its center, shape, and 
    ...
    summary(wdiff)
    ```  
    _The median is -10 and the mean is -14.59.  The distribution is unimodal and mostly symmetric.  Most people want to lose weight, and most people want to lose 10lbs._  

-   Using numerical summaries and a side-by-side box plot, determine if men tend
    to view their weight differently than women.
    ```{r}
    m_weight <- subset(cdc, cdc$gender == "m")
    f_weight <- subset(cdc, cdc$gender == "f")
    m_wdiff <- m_weight$wtdesire - m_weight$weight
    f_wdiff <- f_weight$wtdesire - f_weight$weight
    
    boxplot(m_weight$weight, f_weight$weight, names = c("Male", "Female"), main = "Weight of Males vs Females", ylab = "Weight")
    ...
    ```
    _Men weigh more than women in the study.  The boxplots stay mostly the same for their desired weights, although there are more outliers compressing the IQR.  Although in the difference, we see that women want to lose more weight than men, so we might conclude that men are more comfortable with their current weight than women._  
...
-   Now it's time to get creative. Find the mean and standard deviation of 
    ...
    ```{r}
    w.sd <- sd(cdc$weight)
    w.mean <- mean(cdc$weight)
    w.sd
    w.mean
    
    w.above <- w.mean + w.sd
    w.below <- w.mean - w.sd
    table(cdc$weight > w.below & cdc$weight < w.above)
    14152/20000
    ```

...
1.  Make a histogram of men's heights and a histogram of women's heights.  How 
    ...
    mdims$gender = "male"
    fdims$gender = "female"
    bdims2 <- rbind(mdims, fdims)
    
    ggplot(bdims2, aes(hgt, fill = gender)) +
      geom_histogram(alpha = 0.5, binwidth = 6, position = 'identity') +
      xlab("Height in CM") +
      ylab("Count")
    ```
    
    _Men's heights are more centered at 180cm, while women's heights are centered at around 160cm.  Also, it seems like men's heights have more variance than women's heights.  Both appear vaguely normal._
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    ...
    _Yes, it looks like a normal distribution._
...
3.  Make a normal probability plot of `sim_norm`.  Do all of the points fall on 
    ...
    _The outliers still drift away from the line in the simulation, just as with the actual data._
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    _Yes, relatively.  In almost all situations, the outliers drift, some worse than others, but the bulk of points stick to the line._
...
5.  Using the same technique, determine whether or not female weights appear to 
    ...
    _It's harder to say.  I would say that the weights are not as normally distributed as the heights.  They tend to unravel more, but I'm not sure if it's enough for me to say they're not normally distributed.._
...
6.  Write out two probability questions that you would like to answer; one 
    ...
    _Q1: What is the probability of women whose heights are less than 1 standard deviation from the mean?_
    
    ```{r}
    pnorm(q = (fhgtmean - fhgtsd), mean = fhgtmean, sd = fhgtsd)
    
    sum(fdims$hgt < (fhgtmean - fhgtsd)) / length(fdims$hgt)
    ```
    
    _Q2: What is the probability of women whose weights are greater than 1 standard deviation from the mean?_
    
    ```{r}
    fwgtmean <- mean(fdims$wgt)
    fwgtsd <- sd(fdims$wgt)
    1 - pnorm(q = (fwgtmean + fwgtsd), mean = fwgtmean, sd = fwgtsd)
    sum(fdims$wgt > (fwgtmean + fwgtsd)) / length(fdims$wgt)
    ```
    
    _The weight had a closer agreement._
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    ...
    _Unsure?  If you mean the probability plots for questions C and D, then yes, they seem to have a stepwise pattern because their respective histograms also have a stepwise pattern.  Maybe the data is biased towards younger women, at which point there would also be a difference in chest depth, I suppose._
    
    _If you mean the actual plots C and D, then I don't know.  Both of those normal probability plots look drastically different to the other._
...
-   As you can see, normal probability plots can be used both to assess 
    ...
    _It resembles the normal probability plots of_ `age` _and_ `chest depth` _so I assume it's right skewed._
    
    ```{r}
    hist(fdims$kne.di)
    ```
    
    _Yes, it's right skewed._

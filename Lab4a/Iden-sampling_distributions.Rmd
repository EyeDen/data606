...
1.  Describe this population distribution.
    _This distribution is unimodal with a right skew._
...
2.  Describe the distribution of this sample. How does it compare to the 
    ...
    _The distribution is close to the population.  It's mostly unimodal with a right skew.  Moreover, the median and mean of the sample are only 100 off from the population._
...
3.  Take a second sample, also of size 50, and call it `samp2`.  How does the 
    ...
    _For my samp2, it was closer to the mean at 1552, versus my first sample that had a mean of 1623.  As we increase the sample size, it should become more identical to the population._
...
4.  How many elements are there in `sample_means50`?  Describe the sampling 
    ...
    means?
    _There are 5000 elements.  The center touches the 1500 tick mark, which is the mean of the population.  I wouldn't expect the distribution to change much with more sample means._
...
sample_means_small
```
_There are a 100 elements, and each element represents the mean of a sample of 50 from area._
...
_The center starts to become less unimodal, and the spread begins to narrow towards the center._
...
mean(ames$SalePrice)
```
_The shape is unimodal with a normal distribution. It's center is roughly around 180,000, which is approximately what the small sample of 50 calculated.  The mean of the population is 180796.1_
...
```
_The buckets are larger in this histogram, but still unimodal and normal.  This time I would estimate the mean to be above 180,000._
...
-   Of the sampling distributions from 2 and 3, which has a smaller spread?  If
    ...
    _3 had a smaller spread.  Smaller spread._

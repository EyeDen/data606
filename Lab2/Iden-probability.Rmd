...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    _A streak of 1 is one Hit and one Miss, as shown above.  A streak of 0 is just a Miss._
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    _The streak distribution is unimodal, but with a right skew.  The peak is at 0 streaks, which is also his typical streak length.  His longest streak of baskets was 4._
...
-   Describe the distribution of streak lengths. What is the typical streak 
    ...
    _The number will be different each time I knit this, but usually I've found the distribution to be unimodal with a right skew, peaking at 0.  One of the longest streak of baskets I observed was 6 twice._
...
-   If you were to run the simulation of the independent shooter a second time, 
    ...
    _Because the simulated player is independent, I would expect that, after multiple simulations, the distribution would be somewhat similar in that they will all be unimodal with a heavy right skew, and 0 streaks will be the biggest._
...
-   How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    _Although the number varies, the distribution is almost exactly the same.  If I expect to run the simluation a hundred times, I'd expect to see a similar distribution each time.  If I were to calculate the streak of all of Kobe's games for an NBA season, I would also expect to see a similar distribution each time.  Thus, I have to agree that shooting seems to be independent, and that the hot hand is just a myth._

...
"typical" to mean.
_The distribution is unimodal, mostly normal although it does skew to the right.  By "typical" I assume you mean the mean?  If so, it's roughly 1500.  Other than that, the only other size I can think of is the size of the sample, but that's been outlined above as 60._

2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?
_Given that the sample function is random, I assume it wouldn't be identical to mine, but it should be similar._
...
_I can only think that the data has to have a normal distribution, but that was already stated in the question itself.  I can't think of any other condition._

## Confidence levels

4.  What does "95% confidence" mean? If you're not sure, see Section 4.2.2.
_It means that 95% of the intervals would contain the mean._
...
_Yes, the interval is 1490.828 - 1749.338, and the mean is 1499.69._
...
population mean.
_I would assume that the class size itself should act like a sample, and therefore should have a 95% accuracy rating with 5% falling outside the interval._
...
-   Using the following function (which was downloaded with the data set), 
    ...
    _At the time I ran this, only 1 was outside the range.  1/50 is only a 2% failure, so it's a 98% confidence interval, even though we were aiming for 95%.  That said, this is an estimation so we shouldn't always expect a strict 95% accuracy all the time, and the 99% is inclusive of 95%._

-   Pick a confidence level of your choosing, provided it is not 95%. What is 
    the appropriate critical value?
    _According to the book, a 99% confidence interval's value should be 2.58 instead of 1.96._
...
-   Calculate 50 confidence intervals at the confidence level you chose in the 
    ...
    _At the time I ran this, all values were within my confidence interval.  100% is better than 99%, but not worse, so this is roughly what I expected._
